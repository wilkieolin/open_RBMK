...
for d in l_endian b_endian; do
    ...
    for f in "$LIB/$d"/*; do
        ...
        if [ -e "$DRAGLIB_CACHE/$name" ]; then
            echo "already cached, removing submodule copy: $d/$name"
            rm -f "$f"
        else
            echo "moving to cache: $d/$name"
            mv "$f" "$DRAGLIB_CACHE/$name"
        fi
        moved=$((moved+1))
    done
done
...
echo "moved $moved decompressed librar$([ "$moved" = 1 ] && echo y || echo ies) into $DRAGLIB_CACHE"
